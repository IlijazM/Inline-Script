...
Object.defineProperty(HTMLElement.prototype, 'hasInlineScript', {
  get: function () {
    return this.inlineScript_ !== undefined;
  },
});

Object.defineProperty(HTMLElement.prototype, 'inlineScript', {
  get: function () {
    return this.inlineScript_ !== undefined ? this.inlineScript_ : '';
  },
  set: function (value) {
    ...
    function convert(el, value) {
      ...
      function renameHTMLTagName(element, newTagName) {
        const uniqueClassName =
          ...
          Array(10)
            .fill(0)
            .map((v) => 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'[Math.floor(Math.random() * 36)])
            .join('');
        ...
        outerHTML =
          '<' +
          newTagName +
          outerHTML.substring(1 + tagName.length, outerHTML.length - tagName.length - 1) +
          newTagName +
          '>';
...
let el\n`,
  ...
  addElement(element) {
    ...
    this.script +=
      "el = document.querySelector('." + Array.from(element.classList).find((v) => v.startsWith(UIDPrefix)) + "')\n";

    if (element.inlineScript != '')
      this.script +=
        'el.inlineScript = `' + element.inlineScript.replace(/\\/gm, '\\\\').replace(/\`/gm, '\\`') + '`\n';

    if (element.inlineScriptAttributes.length !== 0) {
      let inlineScriptAttributes = '';
      inlineScriptAttributes += '[\n';
      element.inlineScriptAttributes.forEach((item) => {
        inlineScriptAttributes += `{ name: '${item.name.replace(/\'/gm, "\\'")}', value: '${item.value.replace(
          /\'/gm,
          "\\'"
        )}' },\n`;
      });
